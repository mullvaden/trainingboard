...
namespace Chello.Tests
{
	...
	public class TestTrelloService
	{
        ...
        #region Comments
        [OperationContract]
        [WebGet(ResponseFormat = WebMessageFormat.Json, UriTemplate = "boards/{boardId}/actions?key={key}&filter=commentCard")]
        public List<CommentAction> CommentsForBoard(string key, string boardId)
        {
            //Get Comments for Board
            var comments = data[key].GetRelated<Board, Comment>(boardId).ToList();

            //Turn Comments into CommentActions
            var actions = comments.Select(c => new CommentAction
                                            {
                                                Data = new CommentActionData
                                                           {
                                                               Card = data[key].Get<Card>(c.IdCard),
                                                               Text = c.Text
                                                           },
                                                Date = TestData.CommentDate,
                                                Id = c.Id
                                            }).ToList();
            return actions;
        }
